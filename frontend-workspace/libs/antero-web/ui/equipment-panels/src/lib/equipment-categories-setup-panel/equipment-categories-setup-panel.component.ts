...
import { Category } from '@allmax-angular/antero-web/entities';
...
@Component({
  selector: 'ant-equipment-categories-setup-panel',
  templateUrl: './equipment-categories-setup-panel.component.html',
  styleUrls: ['./equipment-categories-setup-panel.component.scss'],
})
export class EquipmentCategoriesSetupPanelComponent extends PanelBaseComponent implements OnInit, OnDestroy {
  ...
  public ngOnInit(): void {
    ...
    this.subs.push(this.eqStore.selectedList$.subscribe(x => {
      this.isSelectedList = x === EquipmentLists.CATEGORY;
      this.initializeWithValue();
    }))
  }
  ...
  private async getRecords(): Promise<void> {
    this.records = await this.ctrl.getCategories();
  }
  ...
  public async submitEdit(record: Category): Promise<void> {
  ...
  public async submitDelete(record: Category): Promise<void>  {
